...
public class Mapper extends AbsFormat implements Outputable {
	...
	protected String processUpdateSQLContent() {
		...
		sb.append(tab(2)+"</trim>"+line());
		sb.append(tab(2)+"WHERE "+primaryKeyColumnName+" = #{"+columnNameTranslator.translate(primaryKeyColumnName)+"}"+line());
		sb.append(tab(2)+"<include refid=\""+processExtendedWhereClauseSQLId()+"\" />"+line()+tab());
		return sb.toString();
	}
