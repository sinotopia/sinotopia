...
public class StrUtils {
    ...
    /**
     * 将类似key1=123&key2=321&key3=&key4=aaa的字符串解析成map
     *
     * @param text 字符串
     * @return
     */
    public static HashMap<String, String> getKeyValueMap(String text) {
     ...
     * @return
     */
    public static HashMap<String, String> getKeyValueMap(String text, String connector, String divider) {
        HashMap<String, String> map = new HashMap<String, String>();
        if (StrUtils.notEmpty(text)) {
            StringTokenizer tokenizer = new StringTokenizer(text, connector);
            while (tokenizer.hasMoreTokens()) {
                String token = tokenizer.nextToken();
                String key = StrUtils.getMiddleText(token, null, divider);
                if (StrUtils.notEmpty(key)) {
                    String value = StrUtils.getMiddleText(token, divider, null);
                    map.put(key, StrUtils.valueOf(value));
                }
            }
        }
        return map;
...
//			String regex = justNumbers?"\\d+":"-?[\\d]+,?[\\d]+\\.?[\\d]+";
    ...
    public static void main(String[] args) {
        ...
        for (String text : array) {
            System.out.println(text + "\t" + StrUtils.getFirstNumberFromText(text, false));
        }

        for (String text : array) {
            System.out.println(StrUtils.isNumbers(text, false) + "\t" + text);
        }

        String ss = "http://www.baidu.com/aa?aaa=2432424&xx=&bbb=1111=141=14&ccc=999442";

        HashMap<String, String> map = getParametersFromUrl(ss);
