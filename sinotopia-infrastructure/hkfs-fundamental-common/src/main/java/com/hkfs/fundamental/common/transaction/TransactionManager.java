package com.hkfs.fundamental.common.transaction;
...
public class TransactionManager extends DataSourceTransactionManager implements CallbackPreferringPlatformTransactionManager{
	...
	public <T> T execute(TransactionDefinition definition,TransactionCallback<T> callback) throws TransactionException{
		...
		try{
			Object result = callback.doInTransaction(status);//执行逻辑处理，返回Object类型对行啊
			if(result == null){
			...
			if(!status.isCompleted()){//发生异常执行事务回滚
				logger.debug("rollbacking transaction due to runtime exception.["+tid+"]");
				rollback(status);
			}
			throw e;
		}
