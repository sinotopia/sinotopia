...
public class JdtProcessor {
    ...
    public Class process(String filePath) {
        //初始化注释器
        ...
        Class cls = processClass(typeDec, packageName, importList);
        for (TypeDeclaration eachType : innerTypeDec) {
            Class innerCls = processClass(eachType, "", null);
    ...
    protected Class processClass(TypeDeclaration typeDec, String packageName, List importList) {
        //##############获取源代码结构信息#################
        //类注解
        ...
        com.hkfs.fundamental.codegenerator.basis.data.Class cls = new Class(fullClassName);
        ...
        if (superClassType != null) {
            parentCls = new Class(superClassType.toString());
        }
        ...
        if (StrUtils.notEmpty(text)) {
            ...
            for (int i = 0; i < arr.length; i++) {
                ...
                if (arr[i].length() > 0) {
                    ...
                    sb.append(Consts.TAB + Consts.TAB).append(arr[i]);
                }
            }
        }
